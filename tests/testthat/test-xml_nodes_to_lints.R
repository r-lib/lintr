...
test_that("it doesn't produce invalid lints", {
  ...
  xml_nodes_to_lints(
    ...
    {
      expect_identical((.) %>% magrittr::extract2("column_number"), nchar("before") + 1L)
      expect_identical((.) %>% magrittr::extract2("ranges"), list(c(1L, nchar(code))))
    } %>%
    expect_warning(rex::rex("Could not find range start for lint. Defaulting to start of line."))
  ...
  xml_nodes_to_lints(
    ...
    {
      expect_identical((.) %>% magrittr::extract2("column_number"), nchar("before") + 1L)
      expect_identical((.) %>% magrittr::extract2("ranges"), list(c(1L, 1L)))
    } %>%
    expect_warning(rex::rex("Could not find range end for lint. Defaulting to width 1."))
  ...
  xml_nodes_to_lints(
    ...
    {
      expect_identical((.) %>% magrittr::extract2("column_number"), 1L)
      expect_identical((.) %>% magrittr::extract2("ranges"), list(c(1L, nchar(code))))
    } %>%
    expect_warning(rex::rex("Could not find location for lint. Defaulting to start of range."))
})
