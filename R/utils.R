...
set_lang <- function(new_lang) {
  ...
  Sys.setenv(LANGUAGE = new_lang) # nolint: undesirable_function_linter. Avoiding {withr} dep in pkg.
  old_lang
}
...
reset_lang <- function(old_lang) {
  if (is.na(old_lang)) {
    ...
    Sys.setenv(LANGUAGE = old_lang) # nolint: undesirable_function_linter. Avoiding {withr} dep in pkg.
  }
}
