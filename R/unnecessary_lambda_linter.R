...
unnecessary_lambda_linter <- function() {
  ...
  symbol_tests_fmt <- "
    {call_paren_path}/OP-LEFT-PAREN/following-sibling::expr[1][not(preceding-sibling::*[1][self::EQ_SUB])]/SYMBOL
    and SYMBOL_FORMALS = {call_paren_path}/OP-LEFT-PAREN/following-sibling::expr[1]/SYMBOL
    and not(SYMBOL_FORMALS = {call_paren_path}/OP-LEFT-PAREN/following-sibling::expr[position() > 1]//SYMBOL)
  "
  ...
  //SYMBOL_FUNCTION_CALL[ {apply_funs} ]
    ...
    /following-sibling::expr[
      ...
      and (
        (
          { glue(symbol_tests_fmt, call_paren_path = 'expr') }
        ) or (
          { glue(symbol_tests_fmt, call_paren_path = 'expr[OP-LEFT-BRACE and count(expr) = 1]/expr[1]') }
        )
      )
    ]
  ")
