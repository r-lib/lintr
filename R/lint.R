...
lint <- function(filename, linters = NULL, ..., cache = FALSE, parse_settings = TRUE, text = NULL) {
  ...
  for (expr in source_expressions$expressions) {
    for (linter in names(linters)) {
      lints[[itr <- itr + 1L]] <- get_lints(expr, linter, linters[[linter]],
                                            lint_cache, cache, source_expressions$lines)
    }
  }
...
#' @param lint_cache Cache environment.
#' @param cache Logical indicating if the cache should be used to retrieve and cache the lints.
#'
#' @return A list of lints generated by the linter on `expr`.
#'
#' @noRd
get_lints <- function(expr, linter, linter_fun, lint_cache, cache, lines) {
  expr_lints <- NULL
