...
6.  **Implementation Quality**: Is the code readable, clean, efficient, and idiomatic R?
    *   **Centralized State**: For line-by-line properties or multi-variable properties, suggest a single `data.frame` (e.g., `line_metadata`) rather than passing around several parallel vectors.
