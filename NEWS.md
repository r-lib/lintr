...
* `fixed_regex_linter()` gains an option `allow_unescaped` (default `FALSE`) to toggle whether to allow regexes not requiring any escapes or character classes to skip using `fixed = TRUE` (#1689, @MichaelChirico). Thus `fixed_regex_linter(allow_unescaped = TRUE)` would lint on `grepl("[$]", x)` but not on `grepl("a", x)` since the latter does not use any regex special characters.
