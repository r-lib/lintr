...
* `any_duplicated_linter()` highlights the full comparison expression (`==`, `!=`, `<`, `>`) instead of just one operand when linting expressions like `length(unique(x)) == length(x)` or `uniqueN(col) == .N` (#2860, @MichaelChirico).
