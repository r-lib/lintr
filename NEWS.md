...
* New linter `package_hooks_linter()` runs a series of checks also done by `R CMD check` on the `.onLoad()`, `.onAttach()`, `.onUnload()` and `.onDetach()` hooks (#882, @MichaelChirico)
