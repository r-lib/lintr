...
```{r, echo = FALSE}
# ensure output shows 'bad.R'
# NB: knitr doesn't play well with changing directory,
#   so be sure to change back within the same chunk
old_dir <- setwd(tempdir())
bad_code_for_linting <- '
my_func = function(x){
    if(x > 0){
        print("Positive")
    }
...
'
writeLines(bad_code_for_linting, "bad.R")
lintr::lint("bad.R")
unlink("bad.R")
setwd(old_dir)
```
