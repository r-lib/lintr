...
bad_code <- knitr::knit_code$get("bad_code_block")
local({
  code_file <- tempfile()
  on.exit(unlink(code_file))

  writeLines(bad_code, code_file)
  lints <- code_file |>
    lintr::lint() |>
    # ensure output shows 'bad.R', lest the rendering have
    #   randomized filenames (ugly and unnecessary diff)
    lapply(replace, "filename", "bad.R")
  class(lints) <- "lints"
  lints
})
```
