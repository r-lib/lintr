...
These lines (1) generate a Tags section in the documentation for the linter; (2) link to the full table of available linters; and (3) mark the function for export. The most unfamiliar here is probably (1); this uses a helper function inside lintr to generate custom Rd styling. See R/linter_tags.R.
...
closure that will do the actual linting (i.e., they're function factories). We could define additional parameters
...
`Linter()` gives this closure the class 'linter' and guesses its name.
...
for expressing paths within the nested XML data structure. Most linters in `lintr` are built using XPath because it is
concise to write and fast to execute.
...
found in `source_expression`. See `?Lint` and `?xml_nodes_to_lint` for details about the arguments.
