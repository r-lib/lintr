...
In particular, it covers how to write a linter with the intention of contributing it to `lintr`, so
some parts of the below will not be relevant for users writing custom linters for their own packages/repos.
...
These lines (1) generate a Tags section in the documentation for the linter; (2) link to the full table of available linters; and (3) mark the function for export. The most unfamiliar here is probably (1), which can be skipped outside of `lintr` itself. This step uses a helper function inside lintr to generate custom Rd styling. See R/linter_tags.R.
...
for expressing paths within the nested XML data structure. Most linters in `lintr` are built using XPath because it is a powerful language for computation on the abstract syntax tree / parse tree.
...
is very simple & fast to compute, so that the overhead of re-running the linter is low (e.g., `single_quotes_linter`).

Lastly, we have `require_xml = TRUE`. This ensures that the XML representation of the AST is available. This is sometimes
not the case when, for example, `lint()` attempts to parse a script with a syntax error.
...
Here is the core linter logic. `source_expression$xml_parsed_content` is copied to a local variable
(this is not strictly necessary but facilitates debugging). `xpath` is an XPath expression
for expressions matching the discouraged usage. `bad_expr` is the result of executing this logic
on the expression's XML representation.
...
to get accustomed to -- use the plentiful XPath-based linters in `lintr` as a guide to get extra practice.
...
Finally, we pass the matching XML node to `xml_nodes_to_lint()`, which returns `Lint` objects corresponding to any "bad" usages
...
`filter` argument to `devtools::test()`. (NB: this section uses the `assignment_linter()`
...
If your linter is not project-specific you can add it to `default_linters`.
