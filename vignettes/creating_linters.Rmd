...
pipe_call_linter <- function() {
  ...
  Linter(linter_level = "expression", selectors = "%>%", function(source_expression) {
...
`Linter()` creates a `linter` object, which houses the actual linting logic (the closure in the third argument)
and specifies some metadata about the linter.

First, about the closure. It will be called on the `source_expression` variable that
...
Next, about `linter_level = "expression"`. This lets `lint()` return early if `source_expression` is not the expression-level object.
`get_source_expression()` returns an object that parses the input file in two ways -- once is done expression-by-expression, the other
contains all of the expressions in the file. This is done to facilitate caching. Suppose your package has a long source file (e.g.,
100s of expressions) -- rather than run linters on every expression every time the file is updated, when caching is activated `lintr`
...
is very simple & fast to compute, so that the overhead of re-running the linter is low (e.g., `quotes_linter`). In those cases,
use `linter_level = "file"`.

Finally, about `selectors = "%>%"`. This lets `lint()` return early if the expression (or file, depending on `linter_level`)
does not have `%>%`. `selectors` provide necessary-but-not-sufficient conditions on the tokens/text found in `source_expression`
in order to skip expensive checks on irrelevant expressions. Providing `selectors` is optional but can greatly improve performance.
