...
For some use cases, it may be useful to specify linters by string instead of by name, i.e.
`"assignment_linter"` instead of writing out `assignment_linter()`.

Beware that in such cases, you should retrieve the linter function using `call()`, not `get()`:
...
withr::with_tempfile("tmp", {
  ...
  print(as.data.frame(lintr::lint(tmp, linters = get(linter_name)())))

  # linter column is 'assignment_linter'
  print(as.data.frame(lintr::lint(tmp, linters = eval(call(linter_name)))))
})
```
