...
For example, the `any_is_na_linter()` linter detects usages of `any(is.na(x))` and suggests `anyNA(x)` as a more efficient alternative to detect presence *any* of missing values.
...
But, as with other linters in this category, the linter can be customized for any other style guide:
...
One category of linters help you detect some common mistakes statically and provide early feedback.

For example, duplicate arguments in function calls can sometimes cause errors:
...
Even for cases where there will not be a run-time error, this linter explicitly discourages duplicate arguments.
...
This is because duplicate-named objects can be hard to work with programmatically and should typically be avoided.
...
`{lintr}` is designed for extensibility by allowing users to easily create custom linting rules, enabling flexible and tailored code analysis to suit specific coding conventions, style guides, and project requirements.
