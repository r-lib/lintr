...
- linters:
  ...
  [linters](https://lintr.r-lib.org/reference/linters.md) for a full
...
- ...:
  ...
  - [`exclude()`](https://lintr.r-lib.org/reference/exclude.md) (in case
    of `lint()`; e.g. `lints` or `exclusions`)
...
- parse_settings:
  ...
  [settings](https://lintr.r-lib.org/reference/read_settings.md).
  Otherwise, the
...
- exclusions:
  ...
  exclusions for
  [`exclude()`](https://lintr.r-lib.org/reference/exclude.md), relative
  to the package path.
...
Read [`vignette("lintr")`](https://lintr.r-lib.org/articles/lintr.md) to
...
writeLines("a=1", f)
lint(f)
#> /tmp/RtmpOfHpeq/file4a17d689901c0:1:2: style: [assignment_linter] Use one of <-, <<- for assignment, not =.
#> a=1
#>  ^
#> /tmp/RtmpOfHpeq/file4a17d689901c0:1:2: style: [infix_spaces_linter] Put spaces around all infix operators.
#> a=1
#>  ^
unlink(f)
